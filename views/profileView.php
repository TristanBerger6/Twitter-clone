            ...
            <div class="modal__content">
                <form  method="POST" action="" id="formUpdate" enctype="multipart/form-data" >
                    ...
                    <div class="profile-modal__cover"> 
                        <img class="preview-input" id="preview-input6" src='./public/img/cover/<?= $reqUserProfile['imgcover']?>' alt="cover image preview">
                    ...
                    <div class="profile-modal__profile"> 
                        <img class="preview-input" id="preview-input7" src='./public/img/profile/<?= $reqUserProfile['img']?>' alt="profile image preview">
                    ...
                    <div class="profile-modal__inputs flex"> 
                        <input type="text" placeholder="Votre Nom" id="name" name="name" value="<?php if (isset($reqUserProfile['name'])) {echo $reqUserProfile['name'];} ?>" />
                        <input type="text" placeholder="Votre Pseudo" id="username" name="username" value="<?php if (isset($reqUserProfile['username'])) {echo $reqUserProfile['username'];} ?>" />
                        <textarea class="bio" type="text" placeholder="Votre biographie" id="bio" name="bio"><?php if (isset($reqUserProfile['bio'])) {echo $reqUserProfile['bio'];} ?></textarea>
                    </div>
        ...
        <main class='homeContainer'>
            ...
            <div class="profile">
                <div class="profile__cover">
                    <img src='./public/img/cover/<?= $reqUserProfile['imgcover']?>' alt="profile image"> 
                </div>
                <div class="profile__profile flex">
                    <div class="profile__profile__img">
                        <img src='./public/img/profile/<?= $reqUserProfile['img']?>' alt="profile image"> 
                    </div>
                    <?php if($_SESSION['id'] === $_GET['id']){ ?>
                        <button class="modalBtn modalBtn6 basic-btn"  ><p class="fw-700">Editer le profil</p> </button>
                    <?php }else{ ?>
                        <?php if($isFollowed){?>
                        <p class="abo fw-700 basic-btn" user_id='<?= $reqUserProfile['id']?>'>Abonné</p>
                        <?php }else{ ?>
                        <p class="noabo fw-700 basic-btn basic-btn--white" user_id='<?= $reqUserProfile['id']?>'>Suivre</p>
                        <?php } ?>
                    <?php } ?>
                </div>
                <div class="profile__core">
                    <div class="profile__core__name">
                        <p class="fs-600 fw-700"> <?= $reqUserProfile['name']?> </p>
                        <p class="text-light"> @<?= $reqUserProfile['username']?> </p>
                    </div>
                    <p class="profile__core__bio"> <?= $reqUserProfile['bio']?> </p>
                    <p class="text-light"> A rejoint Twitter en <?= $reqUserProfile['date_hour_creation']?> </p>
                    <div class="profile__core__abos flex">
                        <p> <?php echo '<a class="text-light" href="follow/'.$reqUserProfile['id'].'/followed">'?> <span class="text-white"><?= $nbFollowed ?></span> abonnements </a> </p>
                        <p> <?php echo '<a class="text-light" href="follow/'.$reqUserProfile['id'].'/followers">'?> <span class="text-white"><?= $nbFollowers ?></span> abonnées </a></p>
                    </div>
                    
                </div>
            </div>
            <div class="page-switch flex no-padding fs-400 text-light">
                <a class="text-light <?= empty($_GET['type']) ? 'active':''?>" href='profile/<?=$reqUserProfile['id'] ?>'><p>tweets</p></a>
                <a class="text-light <?= $_GET['type'] == 'with_replies' ? 'active':''?>" href='profile/<?=$reqUserProfile['id'] ?>/with_replies'><p>Tweets et réponses</p></a>
                <a class="text-light <?= $_GET['type'] == 'medias' ? 'active':''?>" href='profile/<?=$reqUserProfile['id'] ?>/medias'><p>Médias</p></a>
                <a class="text-light <?= $_GET['type'] == 'likes' ? 'active':''?>" href='profile/<?=$reqUserProfile['id'] ?>/likes'><p>J'aime</p></a>
            </div>
            <?php $i = 0;
